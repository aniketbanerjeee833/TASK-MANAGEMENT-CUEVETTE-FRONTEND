...
export const fetchUser = () => async (dispatch) => {
    const token=JSON.parse(localStorage.getItem("APP-TOKEN"))
      ...
      try {
        const response = await axios.get("https://task-management-cuevette-backend.onrender.co/api/v1/user/me",
          {
            headers:{
    ...
    export const getAllPossibleAssigneeList = () => async (dispatch) => {
      const token=JSON.parse(localStorage.getItem("APP-TOKEN"))
        ...
        try {
          const response = await axios.get("https://task-management-cuevette-backend.onrender.co/api/v1/user/exceptMe",
            {
              headers:{
                Authorization:`Bearer ${token}`
              }
